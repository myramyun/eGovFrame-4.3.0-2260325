  ...
  const initMode = () => {
    switch (props.mode) {
      case CODE.MODE_CREATE:
        setModeInfo({ ...modeInfo, modeTitle: "등록", editURL: "/bbsMaster", });
        break;

      case CODE.MODE_MODIFY:
        setModeInfo({ ...modeInfo, modeTitle: "수정", editURL: `/bbsMaster/${bbsId}`, });
        break;
      default:
  ...
  const updateBoard = () => {
    ...
    if (modeStr === "POST") {
      ...
      for (let key in result) {
        ...
        //console.log("result [%s] ", key, result[key]);
      }
      ...
      <div className="c_wrap">
        ...
        <div className="layout">
          ...
          <div className="contents BOARD_CREATE_REG" id="contents">
            ...
            {modeInfo.mode === CODE.MODE_CREATE && ( <h2 className="tit_2">게시판 생성</h2> )}
            {modeInfo.mode === CODE.MODE_MODIFY && ( <h2 className="tit_2">게시판 수정</h2> )}
            ...
            <div className="board_view2">
              <dl>
                ...
                <dd>
                  <input
                    ...
                    id="bbsNm"
                    placeholder=""
                    defaultValue={result.bbsNm}
                    onChange={(e) => setResult({ ...result, bbsNm: e.target.value }) }
              ...
              <dl>
                <dt>
                  <label htmlFor="bbsIntrcn">게시판 소개</label>
                  <span className="req">필수</span>
                </dt>
                <dd>
                  <textarea
              ...
              <dl>
                <dt>
                  게시판 유형<span className="req">필수</span>
                </dt>
                <dd>
                  {/* 수정/조회 일때 변경 불가 */}
                  {modeInfo.mode === CODE.MODE_CREATE && (
                    <label className="f_select w_130" htmlFor="bbsTyCode">
                      <select
                        ...
                        name="bbsTyCode"
                        title="게시판유형선택"
                        onChange={(e) => setResult({ ...result, bbsTyCode: e.target.value, }) }
                        value={result.bbsTyCode}
                      >
              ...
              <dl>
                <dt>
                  게시판 속성<span className="req">필수</span>
                </dt>
                <dd>
                  {/* 등록 일때 변경 가능 */}
                  {modeInfo.mode === CODE.MODE_CREATE && (
                    <label className="f_select w_130" htmlFor="bbsAttrbCode">
                      <select
                        ...
                        name="bbsAttrbCode"
                        title="게시판속성선택"
                        onChange={(e) => setResult({ ...result, bbsAttrbCode: e.target.value, }) }
                        value={result.bbsAttrbCode}
                      >
                  ...
                  {modeInfo.mode === CODE.MODE_MODIFY && (
                    <span>
                      {result.bbsAttrbCode && getSelectedLabel( bbsAttrbCodeOptions, result.bbsAttrbCode )}
                    </span>
                  )}
                </dd>
              </dl>
              <dl>
                <dt>
                  답장가능여부<span className="req">필수</span>
                </dt>
                <dd>
                  {/* 등록 일때 변경 가능 */}
                  {modeInfo.mode === CODE.MODE_CREATE && (
                    <EgovRadioButtonGroup
                      ...
                      setValue={result.replyPosblAt}
                      setter={(v) => setResult({ ...result, replyPosblAt: v }) }
                    />
                  )}
                  {/* 수정/조회 일때 변경 불가 */}
                  {modeInfo.mode === CODE.MODE_MODIFY && (
                    <span>
                      {result.replyPosblAt && getSelectedLabel( replyPosblAtRadioGroup, result.replyPosblAt )}
                    </span>
                  )}
                </dd>
              </dl>
              <dl>
                ...
                <dd>
                  <EgovRadioButtonGroup
                    ...
                    setValue={result.fileAtchPosblAt}
                    setter={(v) => setResult({ ...result, fileAtchPosblAt: v }) }
                  />
                </dd>
              </dl>
              <dl>
                ...
                <dd>
                  <label className="f_select " htmlFor="posblAtchFileNumber">
                    <select
                      ...
                      title="첨부가능파일 숫자선택"
                      onChange={(e) => setResult({ ...result, posblAtchFileNumber: e.target.value, }) }
                      value={result.posblAtchFileNumber}
              ...
              <div className="board_btn_area">
                <div className="left_col btn1">
                  ...
                  {modeInfo.mode === CODE.MODE_CREATE && (
                    <button className="btn btn_skyblue_h46 w_100" onClick={() => updateBoard()} >
                      저장
                    </button>
                  )}
                  {modeInfo.mode === CODE.MODE_MODIFY && (
                    <>
                      ...
                      <button className="btn btn_skyblue_h46 w_100" onClick={() => { deleteBoardArticle(result.bbsId); }} >
                        사용안함
                      </button>
                    </>
                  )}
