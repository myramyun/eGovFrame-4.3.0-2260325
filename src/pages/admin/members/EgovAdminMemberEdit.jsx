...
function EgovAdminMemberEdit(props) {
  ...
  const retrieveDetail = () => {
    ...
    if (modeInfo.mode === CODE.MODE_CREATE) {
      ...
      setMemberDetail({ tmplatId: "TMPLAT_MEMBER_DEFAULT", groupId: "GROUP_00000000000001", mberSttus: "P", checkIdResult: "중복ID를 체크해 주세요.", });
      retrieveDetailURL = `/members/insert`;
    }
    // 수정이면 초기값 지정 안함
    if (modeInfo.mode === CODE.MODE_MODIFY) { retrieveDetailURL = `/members/update/${uniqId}`; }
    const requestOptions = { method: "GET", headers: { "Content-type": "application/json", }, };
    
    EgovNet.requestFetch(retrieveDetailURL, requestOptions, function (resp) {
      // 수정모드일 경우 조회값 세팅
      if (modeInfo.mode === CODE.MODE_MODIFY) { setMemberDetail(resp.result.mberManageVO); }
      groupCodeOptions = []; //중복 option 값 제거
      //백엔드에서 받은 권한 그룹 options 값 바인딩(아래)
      resp.result.groupId_result.forEach((item) => { groupCodeOptions.push({ value: item.code, label: item.codeNm, name: item.codeDc }); });
  ...
  const formValidator = (formData) => {
    return new Promise((resolve) => {
      ...
      checkIdDplct().then((res) => {
        ...
        if ( formData.get("groupId") === null || formData.get("groupId") === "" ) {
          alert("권한 그룹은 필수 값입니다.");
          return false;
        }
        if ( formData.get("mberSttus") === null || formData.get("mberSttus") === "" ) {
      ...
      <div className="c_wrap">
        ...
        <div className="layout">
          ...
          <div className="contents BOARD_CREATE_REG" id="contents">
            ...
            {modeInfo.mode === CODE.MODE_CREATE && ( <h2 className="tit_2">회원 생성</h2> )}
            {modeInfo.mode === CODE.MODE_MODIFY && ( <h2 className="tit_2">회원 수정</h2> )}
            ...
            <div className="board_view2">
              <dl>
                ...
                <dd>
                  ...
                  {modeInfo.mode === CODE.MODE_CREATE && (
                    <>
                      <input
                        className="f_input2 w_full" type="text" name="mberId" title="" id="mberId" placeholder=""
              ...
              <dl>
                ...
                <dd>
                  ...
                  {modeInfo.mode === CODE.MODE_CREATE && (
                    <input
                      className="f_input2 w_full" type="password" name="password" title="" id="password" placeholder=""
                  ...
                  {modeInfo.mode === CODE.MODE_MODIFY && (
                    <input
                      className="f_input2 w_full" type="password" name="password" title="" id="password" placeholder="빈값이면 기존 암호가 변경되지 않고 그대로 유지됩니다."
                      defaultValue=""
                      onChange={(e) => setMemberDetail({ ...memberDetail, password: e.target.value, }) }
                      ref={(el) => (checkRef.current[1] = el)}
                    />
              ...
              <dl>
                ...
                <dd>
                  <input
                    className="f_input2 w_full" type="text" name="mberNm" title="" id="mberNm" placeholder=""
                ...
                <dd>
                  <label className="f_select w_200" htmlFor="groupId">
                    <select
                      ...
                      name="groupId"
                      title="회원권한유형선택"
                      onChange={(e) => setMemberDetail({ ...memberDetail, groupId: e.target.value, }) }
              ...
              <dl>
                ...
                <dd>
                  <EgovRadioButtonGroup
                    ...
                    setter={(v) => setMemberDetail({ ...memberDetail, mberSttus: v }) }
                  />
                </dd>
              </dl>
              ...
              <div className="board_btn_area">
                <div className="left_col btn1">
                  <button className="btn btn_skyblue_h46 w_100" onClick={() => updateMember()} >
                    저장
                  </button>
                  {modeInfo.mode === CODE.MODE_MODIFY && (
                    <button className="btn btn_skyblue_h46 w_100" onClick={() => { deleteMember(memberDetail.uniqId); }} >
                      삭제
                    </button>
                  )}
                </div>
                <div className="right_col btn1">
                  <Link to={URL.ADMIN_MEMBERS} className="btn btn_blue_h46 w_100" >
                    목록
                  </Link>
                </div>
              </div>
