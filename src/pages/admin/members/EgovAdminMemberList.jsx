    ...
    (srchCnd) => {
      ...
      EgovNet.requestFetch(retrieveListURL, requestOptions, (resp) => {
          ...
          resp.result.resultList.forEach(function (item, index) {
            let authNm = "";
            resp.result.groupId_result.forEach((data) => { if (data.code === item.groupId) authNm = data.codeNm; });
            if (index === 0) mutListTag = []; // 목록 초기화
            const listIdx = itemIdxByPage( resultCnt, currentPageNo, pageSize, index );
            mutListTag.push(
              <Link to={{ pathname: URL.ADMIN_MEMBERS_MODIFY }} state={{ uniqId: item.uniqId, searchCondition: searchCondition, }} key={listIdx} className="list_item" >
                <div>{listIdx}</div>
                <div>{item.mberId}</div>
                <div>{item.mberNm}</div>
                <div>{authNm === "ROLE_ADMIN" ? "관리자" : "사용자"}</div>
                <div>{item.sbscrbDe}</div>
                <div>{item.mberSttus === "P" ? "○" : "✕"}</div>
              </Link>
            );
          });
          ...
          <div className="contents BOARD_CREATE_LIST" id="contents">
            ...
            <div className="condition">
              <ul>
                ...
                <li>
                  <Link to={URL.ADMIN_MEMBERS_CREATE} className="btn btn_blue_h46 pd35" >
                    등록
                  </Link>
                </li>
              </ul>
