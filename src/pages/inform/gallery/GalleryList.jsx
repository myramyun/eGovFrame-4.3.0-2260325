...
/**
 * 갤러리는 일반유저의 등록,수정 가능하므로 단순 로그인 정보만으로 
 * getSessionItem에 권한 확인 필요 없음
 */
              ...
              <ul>
                ...
                <li className="third_2 R">
                  <span className="f_search w_500">
                    <input type="text" name="" placeholder="" defaultValue={searchCondition.searchWrd} ref={wrdRef} onChange={e => { wrdRef.current.value = e.target.value; }} />
                    <button type="button" onClick={() => { handleSearch(cndRef, wrdRef, retrieveList); }} >
                      조회
                    </button>
                  </span>
                </li>
                {user.id && board.bbsUseFlag === "Y" && (
                  <li>
                    <Link to={URL.INFORM_GALLERY_CREATE} state={{ bbsId: bbsId }} className="btn btn_blue_h46 pd35" >
                      등록
                    </Link>
                  </li>
                )}
            ...
            <div className="board_list BRD002">
              ...
              <div className="result">
                ...
                {article.map((item, index) => {
                  ...
                  return (
                    <Link to={{ pathname: URL.INFORM_GALLERY_DETAIL }} state={{ nttId: item.nttId, bbsId: item.bbsId, searchCondition: searchCondition }} key={item.nttId} className="list_item" >
                      <div>{listIdx}</div>
