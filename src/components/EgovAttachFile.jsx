...
function EgovAttachFile({ boardFiles, mode, fnChangeFile, fnDeleteFile, posblAtchFileNumber, }) {
  ...
  function onClickDeleteFile(atchFileId, fileSn, fileIndex) {
    ...
    const requestOptions = { method: "POST", headers: { "Content-type": "application/json", }, body: JSON.stringify({ atchFileId: atchFileId, fileSn: fileSn, }), };
    ...
    boardFiles.forEach(function (item, index) {
      filesTag.push(
        <React.Fragment key = {index}>
          <span>
            <a href={"#LINK"} onClick={function (e) { e.preventDefault(); onClickDownFile(item.atchFileId, item.fileSn); }} download >
              {item.orignlFileNm}
            </a>
        ...
        <div className="file_attach">
          ...
          {mode === CODE.MODE_CREATE && (
            <>
              ...
              <span>총 업로드 가능한 첨부파일 개수는 {posblAtchFileNumber} 개 입니다.</span>
            </>
          )}
          ...
          {mode === CODE.MODE_MODIFY &&
            filesTag.length / 3 < posblAtchFileNumber && (
              <>
                <input name="file_0" id="egovComFileUploader" type="file" multiple onChange={e => onChangeFileInput(e)} ></input>
                <span>현재 업로드 가능한 첨부파일 개수는 {posblAtchFileNumber - filesTag.length / 3} 개 입니다.</span>
              </>
            )}
        </div>
      </dd>
